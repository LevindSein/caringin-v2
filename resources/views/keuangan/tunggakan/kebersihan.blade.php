...
@section('title')
<title>Laporan Tunggakan Kebersihan | BP3C</title>
@endsection

@section('judul')
<h6 class="h2 text-white d-inline-block mb-0">Tunggakan Kebersihan {{$periode}}</h6>
@endsection

@section('button')
<a 
    href="{{url('keuangan/tunggakan/kebersihan')}}"
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="{{url('keuangan/tunggakan/generate')}}" method="POST" target="_blank">
                ...
                <div class="modal-footer">
                    <input type="hidden" name="hidden_data" value="kebersihan"/>
...
$(document).ready(function () {
    var dtable = $('#tabel').DataTable({
        serverSide: true,
		ajax: {
			url: "/keuangan/tunggakan/kebersihan/?periode=" + <?php echo Session::get('periodetagihan')?>,
            cache:false,
		},
		columns: [
    ...
    $('#periode_button').click(function(){
        ...
        window.location.href = "/keuangan/tunggakan/kebersihan?periode=" + periode;
    });
    ...
    $(document).on('click', '.details', function(){
        ...
        $(".divLain").hide();
        $(".divTagihan").hide();
        $(".fasilitas").text("DETAILS KEBERSIHAN");
        $.ajax({
			...
			success:function(data)
			{
                ...
                $(".realisasi-kebersihan").html("<span style='color:green;'>" + data.result.rea_kebersihan.toLocaleString("en-US") + "</span");
                $(".selisih-kebersihan").html("<span style='color:red;'>" + data.result.sel_kebersihan.toLocaleString("en-US") + "</span");
