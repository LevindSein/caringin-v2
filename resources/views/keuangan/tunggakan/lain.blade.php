...
@section('title')
<title>Laporan Tunggakan Lainnya | BP3C</title>
@endsection

@section('judul')
<h6 class="h2 text-white d-inline-block mb-0">Tunggakan Lainnya {{$periode}}</h6>
@endsection

@section('button')
<a 
    href="{{url('keuangan/tunggakan/lain')}}"
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="{{url('keuangan/tunggakan/generate')}}" method="POST" target="_blank">
                ...
                <div class="modal-footer">
                    <input type="hidden" name="hidden_data" value="lain"/>
...
$(document).ready(function () {
    var dtable = $('#tabel').DataTable({
        serverSide: true,
		ajax: {
			url: "/keuangan/tunggakan/lain/?periode=" + <?php echo Session::get('periodetagihan')?>,
            cache:false,
		},
		columns: [
    ...
    $('#periode_button').click(function(){
        ...
        window.location.href = "/keuangan/tunggakan/lain?periode=" + periode;
    });
    ...
    $(document).on('click', '.details', function(){
        ...
        $(".divLain").show();
        $(".divTagihan").hide();
        $(".fasilitas").text("DETAILS LAIN-LAIN");
        $.ajax({
			...
			success:function(data)
			{
                ...
                $(".realisasi-lain").html("<span style='color:green;'>" + data.result.rea_lain.toLocaleString("en-US") + "</span");
                $(".selisih-lain").html("<span style='color:red;'>" + data.result.sel_lain.toLocaleString("en-US") + "</span");
